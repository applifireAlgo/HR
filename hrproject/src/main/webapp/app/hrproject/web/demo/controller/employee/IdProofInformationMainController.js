Ext.define('Hrproject.hrproject.web.demo.controller.employee.IdProofInformationMainController', {
     extend: 'Hrproject.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IdProofInformationMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#IdProofInformationTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#IdProofInformationGrid');
               grid.store.add(data);
               var tree = this.view.down('#IdProofInformationTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#IdProofInformation');
          var grid = this.view.down('#IdProofInformationGrid');
          var tree = this.view.down('#IdProofInformationTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#IdProofInformationGrid');
               ...
               var formPanel = this.getView().up().down("#IdProofInformation");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#IdProofInformationTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IdProofInformation').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#IdProofInformationGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.idCode;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#IdProofInformation').getViewModel().setData(responseData.response.data);
     },
