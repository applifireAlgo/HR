Ext.define('Hrproject.hrproject.web.demo.controller.backgroundcheck.EducationDetailsMainController', {
     extend: 'Hrproject.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.EducationDetailsMainController',
     init: function() {
     ...
     degreeCodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/DegreeType/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#degreeCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'degreeCode', response.responseText);
     ...
     unvCodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/UniversityType/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#unvCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'unvCode', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#unvCode');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#EducationDetailsTree');
               ...
               var compRef = this.view.down('#EducationDetailsTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.eduPk;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#EducationDetails');
          var grid = this.view.down('#EducationDetailsGrid');
          var tree = this.view.down('#EducationDetailsTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#EducationDetailsTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var EducationDetails = this.view.down('#EducationDetailsForm');
               EducationDetails.reset();
               var gridPanel = this.view.down('#EducationDetailsGrid');
          ...
          var formPanel = this.getView().up().down('#EducationDetailsForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#EducationDetailsTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#EducationDetails').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#EducationDetailsGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#EducationDetailsForm').getViewModel().setData(responseData.response.data);
     },
