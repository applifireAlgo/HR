Ext.define('Hrproject.hrproject.web.demo.controller.employee.EmpInformationMainController', {
     extend: 'Hrproject.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.EmpInformationMainController',
     init: function() {
     ...
     designatnTypeCodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#designatnTypeCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'designatnTypeCode', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#designatnTypeCode');
     ...
     visaCodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/Visa/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#visaCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'visaCode', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#visaCode');
     ...
     jobCodeLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#jobCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'jobCode', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#jobCode');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#EmpInformationTree');
               ...
               var compRef = this.view.down('#EmpInformationTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.empId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#EmpInformation');
          var grid = this.view.down('#EmpInformationGrid');
          var tree = this.view.down('#EmpInformationTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#EmpInformationTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var EmpInformation = this.view.down('#EmpInformationForm');
               EmpInformation.reset();
               var gridPanel = this.view.down('#EmpInformationGrid');
          ...
          var formPanel = this.getView().up().down('#EmpInformationForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#EmpInformationTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#EmpInformation').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#EmpInformationGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#EmpInformationForm').getViewModel().setData(responseData.response.data);
     },
