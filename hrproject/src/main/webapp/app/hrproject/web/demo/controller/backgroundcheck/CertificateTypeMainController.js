Ext.define('Hrproject.hrproject.web.demo.controller.backgroundcheck.CertificateTypeMainController', {
     extend: 'Hrproject.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CertificateTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CertificateTypeTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#CertificateTypeGrid');
               grid.store.add(data);
               var tree = this.view.down('#CertificateTypeTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CertificateType');
          var grid = this.view.down('#CertificateTypeGrid');
          var tree = this.view.down('#CertificateTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CertificateTypeGrid');
               ...
               var formPanel = this.getView().up().down("#CertificateType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#CertificateTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CertificateType').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CertificateTypeGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.certCode;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#CertificateType').getViewModel().setData(responseData.response.data);
     },
