Ext.define('Hrproject.hrproject.web.demo.controller.backgroundcheck.UniversityTypeMainController', {
     extend: 'Hrproject.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.UniversityTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#UniversityTypeTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#UniversityTypeGrid');
               grid.store.add(data);
               var tree = this.view.down('#UniversityTypeTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#UniversityType');
          var grid = this.view.down('#UniversityTypeGrid');
          var tree = this.view.down('#UniversityTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#UniversityTypeGrid');
               ...
               var formPanel = this.getView().up().down("#UniversityType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#UniversityTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#UniversityType').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#UniversityTypeGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.unvCode;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#UniversityType').getViewModel().setData(responseData.response.data);
     },
