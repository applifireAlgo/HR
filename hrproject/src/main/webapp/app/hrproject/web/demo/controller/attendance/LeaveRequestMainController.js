Ext.define('Hrproject.hrproject.web.demo.controller.attendance.LeaveRequestMainController', {
     extend: 'Hrproject.view.fw.frameworkController.RegistrationGridViewController',
     alias: 'controller.LeaveRequestMainController',
     init: function() {
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#LeaveRequestGrid');
               grid.store.add(data);
          }
     },
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.leaveRequestId;
          me = this;
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var formPanel = this.getView().down('#LeaveRequest');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#LeaveRequest').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#LeaveRequestGrid').store;
          gridStore.removeAll();
     ...
     saveForm: function(but, evt) {
          ...
          var method;
          if (jsonData.leaveRequestId == null) {
               method = 'POST'
          } else {
               method = 'PUT';
               jsonData.systemInfo = {
