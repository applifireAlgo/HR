Ext.define('Hrproject.hrproject.web.demo.controller.backgroundcheck.CertificationDetailsMainController', {
     extend: 'Hrproject.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CertificationDetailsMainController',
     init: function() {
     ...
     certCodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/CertificateType/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#certCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'certCode', response.responseText);
     ...
     grdCodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/GradeType/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#grdCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'grdCode', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#grdCode');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CertificationDetailsTree');
               ...
               var compRef = this.view.down('#CertificationDetailsTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.certPk;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CertificationDetails');
          var grid = this.view.down('#CertificationDetailsGrid');
          var tree = this.view.down('#CertificationDetailsTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CertificationDetailsTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var CertificationDetails = this.view.down('#CertificationDetailsForm');
               CertificationDetails.reset();
               var gridPanel = this.view.down('#CertificationDetailsGrid');
          ...
          var formPanel = this.getView().up().down('#CertificationDetailsForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#CertificationDetailsTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CertificationDetails').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CertificationDetailsGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#CertificationDetailsForm').getViewModel().setData(responseData.response.data);
     },
