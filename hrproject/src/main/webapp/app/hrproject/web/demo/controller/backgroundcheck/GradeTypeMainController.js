Ext.define('Hrproject.hrproject.web.demo.controller.backgroundcheck.GradeTypeMainController', {
     extend: 'Hrproject.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.GradeTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#GradeTypeTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#GradeTypeGrid');
               grid.store.add(data);
               var tree = this.view.down('#GradeTypeTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#GradeType');
          var grid = this.view.down('#GradeTypeGrid');
          var tree = this.view.down('#GradeTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#GradeTypeGrid');
               ...
               var formPanel = this.getView().up().down("#GradeType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#GradeTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#GradeType').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#GradeTypeGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.grdCode;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#GradeType').getViewModel().setData(responseData.response.data);
     },
