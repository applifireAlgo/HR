Ext.define('Hrproject.hrproject.web.demo.controller.employee.JobTypeMainController', {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#JobType');
          var grid = this.view.down('#JobTypeGrid');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var formPanel = this.getView().up().down("#JobType");
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#JobType').down('#' + me.column.dataIndex + '').store;
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.jobCode;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#JobType').getViewModel().setData(responseData.response.data);
     },
