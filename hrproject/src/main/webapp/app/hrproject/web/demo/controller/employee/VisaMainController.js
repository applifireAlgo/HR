Ext.define('Hrproject.hrproject.web.demo.controller.employee.VisaMainController', {
     extend: 'Hrproject.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.VisaMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#VisaTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#VisaGrid');
               grid.store.add(data);
               var tree = this.view.down('#VisaTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Visa');
          var grid = this.view.down('#VisaGrid');
          var tree = this.view.down('#VisaTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#VisaGrid');
               ...
               var formPanel = this.getView().up().down("#Visa");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#VisaTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Visa').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#VisaGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.visaCode;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Visa').getViewModel().setData(responseData.response.data);
     },
