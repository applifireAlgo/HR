Ext.define('Hrproject.hrproject.web.demo.controller.attendance.AttendenceSummaryMainController', {
     extend: 'Hrproject.view.fw.frameworkController.RegistrationGridViewController',
     alias: 'controller.AttendenceSummaryMainController',
     init: function() {
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#AttendenceSummaryGrid');
               grid.store.add(data);
          }
     },
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.attendenceSummaryId;
          me = this;
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var formPanel = this.getView().down('#AttendenceSummary');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AttendenceSummary').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AttendenceSummaryGrid').store;
          gridStore.removeAll();
     ...
     saveForm: function(but, evt) {
          ...
          var method;
          if (jsonData.attendenceSummaryId == null) {
               method = 'POST'
          } else {
               method = 'PUT';
               jsonData.systemInfo = {
