Ext.define('Hrproject.hrproject.web.demo.controller.backgroundcheck.DegreeTypeMainController', {
     extend: 'Hrproject.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.DegreeTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DegreeTypeTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#DegreeTypeGrid');
               grid.store.add(data);
               var tree = this.view.down('#DegreeTypeTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#DegreeType');
          var grid = this.view.down('#DegreeTypeGrid');
          var tree = this.view.down('#DegreeTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#DegreeTypeGrid');
               ...
               var formPanel = this.getView().up().down("#DegreeType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#DegreeTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#DegreeType').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DegreeTypeGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.degreeCode;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#DegreeType').getViewModel().setData(responseData.response.data);
     },
