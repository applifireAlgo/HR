Ext.define('Hrproject.hrproject.web.demo.controller.attendance.LeaveAllocationMainController', {
     extend: 'Hrproject.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.LeaveAllocationMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#LeaveAllocationTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#LeaveAllocationGrid');
               grid.store.add(data);
               var tree = this.view.down('#LeaveAllocationTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#LeaveAllocation');
          var grid = this.view.down('#LeaveAllocationGrid');
          var tree = this.view.down('#LeaveAllocationTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#LeaveAllocationGrid');
               ...
               var formPanel = this.getView().up().down("#LeaveAllocation");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#LeaveAllocationTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#LeaveAllocation').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#LeaveAllocationGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.leaveCategoryId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#LeaveAllocation').getViewModel().setData(responseData.response.data);
     },
     ...
     saveForm: function(but, evt) {
          ...
          var method;
          if (jsonData.leaveCategoryId == null) {
               method = 'POST'
          } else {
               method = 'PUT';
               jsonData.systemInfo = {
