Ext.define('Hrproject.hrproject.web.demo.controller.attendance.EmployeeLeaveSummaryMainController', {
     extend: 'Hrproject.view.fw.frameworkController.RegistrationGridViewController',
     alias: 'controller.EmployeeLeaveSummaryMainController',
     init: function() {
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#EmployeeLeaveSummaryGrid');
               grid.store.add(data);
          }
     },
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.empLeaveStatusId;
          me = this;
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var formPanel = this.getView().down('#EmployeeLeaveSummary');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#EmployeeLeaveSummary').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#EmployeeLeaveSummaryGrid').store;
          gridStore.removeAll();
     ...
     saveForm: function(but, evt) {
          ...
          var method;
          if (jsonData.empLeaveStatusId == null) {
               method = 'POST'
          } else {
               method = 'PUT';
               jsonData.systemInfo = {
