Ext.define('Hrproject.hrproject.web.demo.controller.documentmanager.DocumentTypeMainController', {
     extend: 'Hrproject.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.DocumentTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DocumentTypeTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#DocumentTypeGrid');
               grid.store.add(data);
               var tree = this.view.down('#DocumentTypeTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#DocumentType');
          var grid = this.view.down('#DocumentTypeGrid');
          var tree = this.view.down('#DocumentTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#DocumentTypeGrid');
               ...
               var formPanel = this.getView().up().down("#DocumentType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#DocumentTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#DocumentType').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DocumentTypeGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.docTypeCode;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#DocumentType').getViewModel().setData(responseData.response.data);
     },
