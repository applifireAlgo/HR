Ext.define('Hrproject.hrproject.web.demo.controller.employee.EmpIdProofMainController', {
     extend: 'Hrproject.view.fw.frameworkController.RegistrationGridViewController',
     alias: 'controller.EmpIdProofMainController',
     init: function() {
     ...
     idCodeLoad: function() {
          ...
          Ext.Ajax.request({
               url: 'secure/IdProofInformation/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#idCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'idCode', response.responseText);
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null) {
               var grid = this.view.down('#EmpIdProofGrid');
               grid.store.add(data);
          }
     },
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.empIdpk;
          me = this;
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var formPanel = this.getView().down('#EmpIdProof');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#EmpIdProof').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#EmpIdProofGrid').store;
          gridStore.removeAll();
