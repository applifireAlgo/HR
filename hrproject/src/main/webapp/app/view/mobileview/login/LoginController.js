Ext.define('Hrproject.view.mobileview.login.LoginController', {
			...
			Ext.Ajax.request({
				...
				success : function(response, scope) {
					...
					if (jsonResponse.response.success) {
						...
						scope.container.add(Ext.create('Hrproject.view.mobileview.main.MainPanel'));
					} else {
						if (jsonResponse.response.changePassword == true) {
							Ext.Msg.confirm('Confirm', jsonResponse.response.message, function(id, value) {
								if (id == 'yes') {
									...
									scope.container.removeAll();
									scope.container.add(Ext.create('Hrproject.view.mobileview.login.ChangePasswordScreen'));
								}
							}, {
								controller : this,
								scope : scope
		...
		Ext.Ajax.request({
			...
			success : function(response, sender) {
				...
				if (responseText.response.success) {
					...
					var forgetPasswordWindow = Ext.create('Hrproject.view.mobileview.login.ForgetPasswordWindow', {
